...
export default function SearchScreen() {
  ...
  const waterResults = useMemo(() => {
    ...
    const dams = DAMS.filter((d) => d.name.toLowerCase().includes(q) || d.region.toLowerCase().includes(q))
      .slice(0, 10)
      .map((d) => ({ id: d.id, name: d.name, sub: d.region, kind: 'dam' as const, item: d }));
    const rivers = RIVERS.filter((r) => r.name.toLowerCase().includes(q) || r.region.toLowerCase().includes(q))
      .slice(0, 10)
      .map((r) => ({ id: r.id, name: r.name, sub: r.region, kind: 'river' as const, item: r }));
    return [...dams, ...rivers];
  }, [query2]);
  ...
  const speciesResults = useMemo(() => {
    ...
    return speciesList.filter(
      (s) => s.nameBg.toLowerCase().includes(q) || s.nameLatin.toLowerCase().includes(q)
    );
  }, [query2]);
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <Text style={styles.empty}>
              {query2.length < 2
                ? 'Въведи поне 2 букви за търсене на рибари'
                : searching ? '' : 'Няма намерени рибари'}
            </Text>
          }
          renderItem={({ item }) => (
            <Pressable
              ...
              onPress={() =>
                navigation.navigate('Main', {
                  ...
                  params:
                    item.kind === 'dam'
                      ? { focusDamId: item.id }
                      : { focusRiverId: item.id },
                })
              }
            >
              <View style={styles.iconWrap}>
                <Ionicons
                  name={item.kind === 'dam' ? 'layers-outline' : 'git-branch-outline'}
                  size={20}
                  color={item.kind === 'dam' ? colors.primary : '#2E9B5A'}
                />
              </View>
              <View style={{ flex: 1 }}>
