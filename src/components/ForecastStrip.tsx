...
type Day = { date: string; code: number; maxC: number };
...
export function ForecastStrip({ latitude, longitude, cacheKey }: Props) {
  ...
  const styles = useMemo(
    () =>
      StyleSheet.create({
        ...
        t: { ...typography.bodyBold, color: colors.primary, marginTop: 4 },
  ...
  useEffect(() => {
    let cancelled = false;
    const storageKey = `@ribolov/forecast:${cacheKey}`;
    ...
    (async () => {
      ...
      try {
        ...
        const url = `https://api.open-meteo.com/v1/forecast?latitude=${latitude}&longitude=${longitude}&daily=weather_code,temperature_2m_max&forecast_days=7&timezone=auto`;
    ...
    <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.row}>
      {days.map((d) => (
        <View key={d.date} style={styles.chip}>
          ...
          <Text style={styles.t}>{d.maxC}°</Text>
